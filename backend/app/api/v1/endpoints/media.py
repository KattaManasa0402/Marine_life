...
from app.tasks.ai_tasks import process_media_for_ai_sync_test  # ✅ UPDATED IMPORT
...
@router.post(
    ...
    description="Allows an authenticated user to upload a media file (photo or video) and associated metadata."
)
async def upload_new_media(
    ...
    if not minio_service.minio_client:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Object storage service is not available.")
    
    if not file.content_type or not (file.content_type.startswith("image/") or file.content_type.startswith("video/")):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid file type. Only images and videos are allowed.")
    
    try:
        file_url = await minio_service.upload_file_to_minio(file=file)
        if not file_url:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not upload file to object storage.")
        ...
        if sighting_timestamp_str:
            try:
                ...
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid sighting_timestamp format. Use ISO 8601 format.")
        ...
        # --- ✅ TRIGGER UPDATED CELERY TASK ---
        if created_media_item:
            print(f"Sending AI processing task for media_item_id: {created_media_item.id}, URL: {created_media_item.file_url}")
            process_media_for_ai_sync_test.delay(
                media_item_id=created_media_item.id, 
                file_url=created_media_item.file_url
            )
        # --- ✅ END OF UPDATED CELERY TASK ---

        return created_media_item

    except minio_service.S3Error as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Storage error during upload: {e}")
    except HTTPException:
        raise
    except Exception as e:
        print(f"An unexpected error occurred in upload_new_media: {e}")
        import traceback; traceback.print_exc()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An unexpected error occurred while processing the upload.")
...
async def read_media_items_list(
    ...
    user_id: Optional[int] = None, species_filter: Optional[str] = None 
):
...
async def read_single_media_item(
    media_item_id: int, db: AsyncSession = Depends(get_db)
):
...
async def update_existing_media_item(
    media_item_id: int, media_update_in: schemas.MediaItemUpdate,
...
async def delete_single_media_item(
    media_item_id: int, db: AsyncSession = Depends(get_db), current_user: UserModel = Depends(get_current_active_user)
):
    db_media_item = await crud.crud_media.get_media_item(db, media_item_id=media_item_id)
    if db_media_item is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Media item not found")
    ...
    if deleted_db_item is None: 
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Media item was found but could not be deleted from database.")
    return None
